...
window.onunload=function() {
	var d_i = dd.elements.length; while(d_i--)
	{
		...
		for(var d_x in d_o)	d_o[d_x] = null;
		dd.elements[d_o.name] = null;
	}
}
...
{
    if ((Browser.Platform.ios || Browser.Platform.android) && document.ontouchstart != d_x)
        document.ontouchstart = d_x;
    else if(document.onmousedown != d_x)
	{
...
{
    if ((Browser.Platform.ios || Browser.Platform.android) && document.ontouchmove != d_x)
        document.ontouchmove = d_x;
	else if(document.onmousemove != d_x)
	{
...
{
    if ((Browser.Platform.ios || Browser.Platform.android) && document.ontouchend != d_x)
        document.ontouchend = d_x;
	else if(document.onmouseup != d_x)
	{
...
dd.evt = function(d_e)
{
    if (Browser.Platform.ios || Browser.Platform.android)
        this.e = d_e.targetTouches[0] || window.event;
    else
        this.e = d_e || window.event;
	this.but = this.e.which || this.e.button || 0;
    this.button = (this.e.type == 'mousedown' || this.e.type == 'touchstart')? this.but
...
function DDObj(d_o, d_i)
{
	...
	this.contenttype		= 0; // current type in content
	this.contentsize		= 1; // current size of content
	this.contentsizebase	= 1; // basic size of content (if empty)
	this.contenttypeaccept	= 0; // fix accepting types
	this.contentsizeaccept	= 0; // current accepting size of content
	this.enchanttype		= 0;
	this.tooltip			= ''; // current tooltip
    this.iid				= 0; // current itemID
    this.preis				= 0; // current price
    this.preis2				= 0; // current price
    this.premium			= 0; // is premium
    this.quest				= 0; // is questitem
	this.contentdraggable	= 1;
	this.contentdroppable	= 1;
	this.unique	= 0;

    this.hasHighlightDiv = false;
...
function PICK(d_ev)
{
    if (Browser.Platform.ios || Browser.Platform.android)
    {
        if (typeof blackoutScreen == 'undefined' || !blackoutScreen)
        {
            dd.touchEvent = d_ev;
        }
        hideTooltips();
	...
	while(d_i--)
	{
		...
		{
			if(d_o.z > d_cmp && d_o.tooltip != '' && (d_t = dd.e.src.tag).indexOf('inpu') < 0 && d_t.indexOf('texta') < 0 && d_t.indexOf('sele') < 0 && d_t.indexOf('opti') < 0 && d_t.indexOf('scrol') < 0)
			{
				d_cmp = d_o.z;
				dd.obj = d_o;
			}
...
function DROP(d_ev)
{
	if(dd.obj)
	{
        ...
        if (Browser.Platform.ios || Browser.Platform.android)
        {
            if (!dd.msmoved && dd.touchEvent)
            {
                dd.obj.div.onmouseover(dd.touchEvent.targetTouches[0]);
...
DDObj.prototype.setHighlight = function(state)
{
	if(!this.hasHighlightDiv)
	{
		var mainDiv = document.createElement("div");
		mainDiv.id = this.id + '_high';
		mainDiv.className = "dd_highlight";
		this.div.appendChild(mainDiv);

		this.hasHighlightDiv = true;
	}
	if(state)
		$(this.id + '_high').style.display = 'block';
	else
		$(this.id + '_high').style.display = 'none';
}